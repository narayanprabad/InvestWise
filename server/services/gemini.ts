...
// Use Gemini 1.5 Pro for advanced reasoning capabilities
...
export async function getInvestmentAdvice(
  ...
  try {
    ...
    const prompt = `You are an expert investment advisor for a financial technology company.
...
YOUR RESPONSE MUST BE VALID JSON with no other text before or after the JSON. The JSON must match this exact schema:
...
Now, provide a detailed investment response to this query: ${query}`;
